...
- "Cómo funciona": interactivo (`app/components/ComoFunciona.tsx`) — 3 pasos a la izquierda, clic cambia el panel animado de la derecha (mockup + CSS keyframes en `globals.css`, sin librería de animación). Reinicia la animación por `key`/render condicional al cambiar de paso. Sin dependencias nuevas a propósito.
